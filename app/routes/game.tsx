...
export async function loader({ context, params }: Route.LoaderArgs) {
	...
	try {
		...
		details = await api.getGameStoreDetails(appid.toString());
		details = details[game.appid.toString()].data as SteamGameDetails;
	} catch (err) {
            ...
            <div
                ...
                <div className="absolute inset-0 flex items-end p-8">
                    <div className="flex justify-between items-end w-full">
                        <div>
                            <h1 className="text-5xl font-bold text-emerald-300 mb-2">{game.name}</h1>
                            <div className="flex gap-4 text-sm text-emerald-200">
                                {details?.release_date && (
                                    <span>
                                        Released: {new Date(details.release_date.date).toLocaleDateString()}
                                    </span>
                                )}
                                {details?.metacritic && (
                                    <a
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-1 space-y-6">
                            ...
                            <div className="bg-emerald-900/30 p-4 rounded-lg border border-emerald-700/40">
                                <h3 className="font-semibold mb-2 text-emerald-300">Your Stats</h3>
                            ...
                            {details?.controller_support && (
                                <div className="bg-slate-900 p-4 rounded-lg border border-emerald-700/40">
                                    <h3 className="font-semibold mb-2 text-emerald-300">Controller Support</h3>
                                    <p className="text-sm text-slate-100">{details.controller_support}</p>
                                </div>
                            )}
                            ...
                            {details?.website && (
                                <div className="bg-slate-900 p-4 rounded-lg border border-emerald-700/40">
                                    <h3 className="font-semibold mb-2 text-emerald-300">Official Website</h3>
                                    <a
                            ...
                            {details?.pc_requirements && (
                                <div className="bg-slate-900 p-4 rounded-lg border border-emerald-700/40">
                                    <h3 className="font-semibold mb-3 text-emerald-300">PC Requirements</h3>
                                    <div>
                                        <h4 className="text-sm font-medium mb-2 text-emerald-200">
                                            Minimum
                                        </h4>
                                        <div className="text-xs text-slate-100 space-y-1 mb-3">
                                            <div dangerouslySetInnerHTML={{ __html: details.pc_requirements.minimum }} />
                                        </div>
                                    </div>
                                    {details.pc_requirements.recommended && (
                                        <div>
                                            ...
                                            <div className="text-xs text-slate-100 space-y-1">
                                                <div dangerouslySetInnerHTML={{ __html: details.pc_requirements.recommended }} />
                                            </div>
                                        </div>
                                    )}
                                </div>
                            ...
                            {details?.categories && details.categories.length > 0 && (
                                <div className="bg-slate-900 p-4 rounded-lg border border-emerald-700/40">
                                    <h3 className="font-semibold mb-2 text-emerald-300">Categories</h3>
                        ...
                        <div className="lg:col-span-2 space-y-8">
                            {/* Description */}
                            <div>
                                <h2 className="text-2xl font-bold mb-4 text-emerald-300">About</h2>
                                <div
                                    ...
                                    {details?.detailed_description ? (
                                        <div dangerouslySetInnerHTML={{ __html: details.detailed_description }} />
                                    ) : (
                            ...
                            {details?.screenshots && details.screenshots.length > 0 && (
                                <div>
                                    <h2 className="text-2xl font-bold mb-4 text-emerald-300">Screenshots</h2>
